...
## 現行BGM（2026年9月10日差し替え）

ユーザー指定により、作品17の一覧・ギャラリー・紹介ページのBGMを「一夜限りの肝試し」に差し替えた。

提供元は `/Users/guarhiro/Downloads/一夜限りの肝試し.mp3`。約66.88秒、48 kHz・ステレオのMP3を再エンコードせず、`public/media/kimodameshi/one-night-kimodameshi.mp3` にそのままコピーした。既存の再生・停止操作、ループ再生、フェードインを使用する。

元ファイル・掲載用ファイル・公開用出力のバイト一致と音声全体のデコードを確認。公開用HTMLで新しい曲名・音源URL・ループ設定・自動再生なしを確認した。

初期採用曲とA案・B案・C案の制作記録は履歴として保持する。
