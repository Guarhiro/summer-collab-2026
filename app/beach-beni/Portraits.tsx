...
export default function Portraits({ id, name }: { id: string; name: string }) {
  ...
  return (
    <div className={styles.portraits}>
      <img id={`${id}-portrait`} className={styles.portrait} src={`${basePath}/media/beach-beni/${id}-${selected}.png`} width="768" height="1280" alt={`${name}のイラスト ${selected}`} loading="lazy" />
      <div className={styles.choices} aria-label={`${name}の画像切り替え`}>
        {[1, 2].map((number) => (
          <button type="button" key={number} aria-pressed={selected === number} aria-controls={`${id}-portrait`} aria-label={`${name}の画像${number}を表示`} onClick={() => setSelected(number)}>
            <img src={`${basePath}/media/beach-beni/${id}-${number}.png`} alt="" width="60" height="100" loading="lazy" /><span>0{number}</span>
          </button>
        ))}
      </div>
    </div>
